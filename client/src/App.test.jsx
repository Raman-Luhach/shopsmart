...
// ── Mocks ──────────────────────────────────────────────────────────────────
const mockHealth     = { status: 'ok', message: 'ShopSmart Backend is running', timestamp: '2024-01-01T00:00:00Z' };
const mockCategories = { categories: [{ id: 1, name: 'Electronics', icon: '💻' }] };
const mockProducts   = {
    products: [
        { id: 1, name: 'Wireless Headphones', category: 'Electronics', price: 299.99, rating: 4.8, reviews: 1240, image: 'headphones', badge: 'Best Seller' },
    ],
    total: 1,
};
...
beforeEach(() => {
    global.fetch = vi.fn((url) => {
        if (url.includes('/api/health'))     return Promise.resolve({ json: () => Promise.resolve(mockHealth) });
        if (url.includes('/api/categories')) return Promise.resolve({ json: () => Promise.resolve(mockCategories) });
        if (url.includes('/api/products'))   return Promise.resolve({ json: () => Promise.resolve(mockProducts) });
